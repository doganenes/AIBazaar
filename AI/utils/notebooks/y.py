...
base_url = "https://www.epey.com/akilli-telefonlar/e/YTo9OntpOjE4NzA7YTozOntpOjA7czo3OiIyMTk5NDE1IjtpOjE7czo3OiIyNTE1MDkzIjtpOjI7czo3OiIyMzg2NTQ3Ijt9fQ==/"
...
for page_num in range(1, 2):
    ...
    if container:
        ...
        for ul in ul_elements[2:]:
            a_tag = ul.find("a", class_="urunadi")
            if not a_tag:
                continue
            print(a_tag)    
            product_title = a_tag.get("title")  # ✅ Eksik olan tanım
            product_link = a_tag.get("href")
            print(product_link)

            driver.get(product_link)
            phoneModel = driver.find_element(By.CSS_SELECTOR, ".baslik h1").text.split(" (")[0]
            print(phoneModel)
            ...
            all_phones.append(description)

    driver.quit()

df = pd.DataFrame(all_phones, columns=["description"])
df.to_csv("epey_id_ile_nokta_atisi1.csv", index=False)
