...
def advanced_feature_engineering(df):
    ...
    df["battery_display_ratio"] = df["battery"] / (df["display_size"] + 0.1)  # 0 bölme hatası önleme
    ...
    df["tech_generation"] = np.where(df["chipset"] <= 14, 0,  # Eski teknoloji
                                   np.where(df["chipset"] <= 7, 1,   # Orta teknoloji
                                          np.where(df["chipset"] <= 5, 2, 3)))  # Yeni teknoloji
...
def predict_product_rf(request):
    try:
        ...
        # Veri yükleme
        df = pd.read_csv(r"C:\Users\pc\Desktop\AIbazaar\AIBazaar\AI\utils\notebooks\Product.csv")
