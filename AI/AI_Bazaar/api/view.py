...
def predict_product_rf(request):
    ...
    try:
        ...
        cv_folds = min(
            10, max(3, len(df_clean) // 15)
        )  # Veri büyüklüğüne göre fold sayısı
        cv = KFold(n_splits=cv_folds, shuffle=True, random_state=42)
        ...
        similar_phones = find_similar_phones(
            df_clean, prediction, os, user_specs, top_n=3
        )

        # Özellik önemlilikleri (sadece gradient boosting için)
        try:
            gb_importance = dict(
                zip(feature_columns, model.named_estimators_["gb"].feature_importances_)
            )
            rf_importance = dict(
                zip(feature_columns, model.named_estimators_["rf"].feature_importances_)
            )
            ...
            for feature in feature_columns:
                combined_importance[feature] = (
                    gb_importance[feature] + rf_importance[feature]
                ) / 2
            ...
            top_features = {
                "ensemble_model": "Feature importance not available for voting regressor"
            }

        print(f"Model Performansı - R2: {mean_r2:.4f} (±{std_r2:.4f})")
        print(
            f"Eğitim R2: {train_r2:.4f}, MAE: {train_mae:.2f}, RMSE: {train_rmse:.2f}"
        )
        ...
        response_data = {
            ...
            "model_performance": {
                "cross_validation": {
                    ...
                    "mae_mean": round(
                        -cv_results["neg_mean_absolute_error"]["mean"], 2
                    ),
                },
                "training_metrics": {
                    ...
                    "mape": round(
                        np.mean(np.abs((y - y_pred_train) / y)) * 100, 2
